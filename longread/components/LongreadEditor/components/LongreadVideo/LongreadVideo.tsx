...
const LongreadVideo: React.FC<LongreadVideoProps> = ({
  item,
  onBlur,
  onFocus,
  editable,
  ...
  const [videoLessonItem, setVideoLessonItem] =
    useState<IAutoCompleteVideoLessonItem | null>(null);
  const [savedVideoUrl, setSavedVideoUrl] = useState('');
  const [savedVideoTitle, setSavedVideoTitle] = useState('');
  const [searchValue, setSearchValue] = useState('');
  const [autoCompleteValue, setAutoCompleteValue] = useState(
    item?.lessonResource?.title.trim() || '',
  );
  ...
  const handleApplyButtonClick = useCallback(() => {
    ...
    videoChange(
      videoLessonItem?.meta.title,
      videoId,
      videoLessonItem?.meta.videoUrl,
      videoLessonItem?.id,
    );
  }, [
    videoChange,
    ...
    <div className={css.root}>
      {editable && (
        <>
          <div className={css.description}>
            Найдите гранулу c нужным видео. Видео в грануле должно быть
          ...
          <div className={css.top}>
            <AutoComplete
              ...
              onSelect={(value, option) =>
                setVideoLessonItem(option as IAutoCompleteVideoLessonItem)
              }
            />
            <Button
              type="primary"
      ...
      {(item?.lessonResource?.videoId || savedVideoUrl) && (
        <>
          ...
          {isError && (
            <Text type="danger">
              Не удалось подгрузить видео. Проверьте, что выбрали верную гранулу
            </Text>
          )}
        </>
      )}
